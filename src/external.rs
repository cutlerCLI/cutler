...
fn substitute_arg(arg: &str, variables: Option<&toml::value::Table>) -> Vec<String> {
    // Only substitute if the argument starts with '$'
    if arg.starts_with('$') {
        // Determine the key name. Support either "$key" or "${key}".
        ...
        // First, try to find the variable in our [external.variables] table.
        if let Some(vars) = variables {
            if let Some(var_value) = vars.get(key) {
                if let Some(arr) = var_value.as_array() {
                    ...
                    return vec![var_value.to_string()];
                }
            }
        }
        // Fall back: try to get the variable from the process environment.
        if let Ok(env_val) = env::var(key) {
            return vec![env_val];
        }
    }
    // Otherwise, no substitution needed – leave the argument unchanged.
    vec![arg.to_string()]
}
...
pub fn execute_external_commands(
    ...
    // Get the [external] section.
    if let Some(ext_section) = config.get("external") {
        // Extract the optional variables table.
        let variables = ext_section.get("variables").and_then(|v| v.as_table());

        // Look for the command array under the key "command".
        if let Some(commands) = ext_section.get("command") {
                        ...
                        if let Some(arr) = arg_val.as_array() {
                            ...
                            for elem in arr {
                                if let Some(arg_str) = elem.as_str() {
                                    let substituted = substitute_arg(arg_str, variables);
                            ...
                            return Err(format!(
                                "Invalid type for 'args' in external command '{}'",
                                cmd
                            )
                            .into());
                        }
