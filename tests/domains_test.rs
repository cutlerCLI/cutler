...
mod tests {
    ...
    use cutler::domains::convert::{prefvalue_to_toml, toml_to_prefvalue};
    ...
    async fn test_collect_domains_nested() {
        ...
        // When TOML creates nested structures, they're only flattened if valid domains
        // Since "com.apple.root.nested" won't be a valid domain, this should NOT flatten
        ...
        let domains = collect(&config).await.unwrap();
        assert_eq!(domains.len(), 1);
        // Should have "root" domain with "nested" as a table value (not flattened)
        let got = domains.get("root").unwrap();
        let nested_table = got.get("nested").unwrap().as_table().unwrap();
        assert_eq!(
...
"#,
        ...
        // NSGlobalDomain subdomains are fully flattened
